    ...
    if (!property) {
          return (
                  <div className="text-center py-12">
                          <h2 className="text-xl font-semibold text-gray-900">Property not found</h2>h2>
    ...
    return (
          <div className="space-y-6">
                <div className="flex items-center space-x-4">
                        ...
                        <h1 className="text-2xl font-bold text-gray-900">{property.name}</h1>h1>
                </div>div>
          
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 bg-white rounded-lg shadow p-6">
                                  <h2 className="text-lg font-semibold mb-4">Property Details</h2>h2>
                                  <div className="grid grid-cols-2 gap-4">
                                              <div className="flex items-start space-x-3">
                                                            <MapPin className="h-5 w-5 text-gray-400" />
                                                            <div>
                                                                            <p className="text-sm text-gray-500">Address</p>p>
                                                                            <p className="font-medium">{property.address || 'Not specified'}</p>p>
                                                            </div>div>
                                              </div>div>
                                              <div className="flex items-start space-x-3">
                                                            ...
                                                            <div>
                                                                            ...
                                                                            <p className="font-medium">{property.rentAmount ? `$${property.rentAmount.toLocaleString()}` : 'N/A'}</p>p>
                                                            </div>div>
                                              </div>div>
                                  </div>div>
                        </div>div>
                        ...
                        <div className="bg-white rounded-lg shadow p-6">
                                  <h2 className="text-lg font-semibold mb-4">Documents</h2>h2>
                          {propertyDocs.length === 0 ? (
                        <p className="text-gray-500 text-sm">No documents available</p>p>
                      ) : (
                        <ul className="space-y-3">
                          {propertyDocs.map(doc => (
                                          <li key={doc.id} className="flex items-center space-x-3">
                                                            <FileText className="h-5 w-5 text-blue-500" />
                                                            <span className="text-sm">{doc.name}</span>span>
                                          </li>li>
                                        ))}
                        </ul>ul>
                                  )}
