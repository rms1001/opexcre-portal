  ...
  if (loading) {
    ...
    return (
          <div className="space-y-8">
                <div>
                        <h1 className="text-2xl font-bold text-gray-900">Welcome, {userProfile?.name || 'Client'}</h1>h1>
                        <p className="text-gray-600">Access your properties, documents, and reports.</p>p>
                </div>div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <Link to="/portal/properties" className="bg-white rounded-xl border border-gray-200 p-6 hover:border-emerald-300 transition-colors">
                                  ...
                                  <h3 className="font-semibold text-gray-900 mb-1">Properties</h3>h3>
                                  <p className="text-sm text-gray-500">{properties.length} properties assigned</p>p>
                        </Link>Link>
                        ...
                        <Link to="/portal/documents" className="bg-white rounded-xl border border-gray-200 p-6 hover:border-emerald-300 transition-colors">
                                  ...
                                  <h3 className="font-semibold text-gray-900 mb-1">Documents</h3>h3>
                                  <p className="text-sm text-gray-500">{documents.length} documents available</p>p>
                        </Link>Link>
                        ...
                        <Link to="/portal/reports" className="bg-white rounded-xl border border-gray-200 p-6 hover:border-emerald-300 transition-colors">
                                  ...
                                  <h3 className="font-semibold text-gray-900 mb-1">Reports</h3>h3>
                                  <p className="text-sm text-gray-500">View portfolio reports</p>p>
                        </Link>Link>
                </div>div>
            ...
            {properties.length > 0 && (
                    <div className="bg-white rounded-xl border border-gray-200">
                              <div className="p-6 border-b border-gray-200">
                                          <h2 className="text-lg font-semibold text-gray-900">Your Properties</h2>h2>
                              </div>div>
                                ...
                                {properties.slice(0, 3).map((property) => (
                                    <Link key={property.id} to={`/portal/properties/${property.id}`} className="flex items-center gap-4 p-4 hover:bg-gray-50">
                                                    ...
                                                    <div className="flex-1">
                                                                      <p className="font-medium text-gray-900">{property.name}</p>p>
                                                                      <p className="text-sm text-gray-500">{property.address}</p>p>
                                                    </div>div>
                                    </Link>Link>
                                  ))}
                              </div>div>
